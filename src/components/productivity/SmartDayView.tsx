  ...
  if (loading) {
    return (
      <div className="space-y-4">
        {[1, 2].map((i) => (
          <div key={i} className="h-24 rounded-xl bg-[#F0F1F5] dark:bg-[#1A1D27] animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  if (!hasClasses) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-xl border-2 border-dashed border-[#D3D3D3] dark:border-[#2E3347] bg-white dark:bg-[#1A1D27] p-10 flex flex-col items-center justify-center gap-4 text-center">
          <div className="space-y-1.5">
            <p className="text-base font-semibold text-[#000000] dark:text-[#F5F6F8]">No classes here yet</p>
